...
// Prepare and execute the query to retrieve the user's role ID, hashed password, ID, and status
$query = $conn->prepare("SELECT id, role_id, password, email, username, status FROM users WHERE username =?");
if (!$query) {
...
if ($user_data) {
    // Check if the user's status is active (1)
    if ($user_data['status'] == 0) {
        send_response(false, 'Your account is inactive. Please contact the administrator.');
    } else {
